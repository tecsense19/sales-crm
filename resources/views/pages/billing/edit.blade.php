...
@section('content')
    ...
    <form action="{{ route('billing.update', $billing) }}" method="POST" class="mx-auto space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="border-b border-gray-100 px-7 py-5 dark:border-gray-800 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Update billing record.</p>
                </div>
                <div class="text-right">
                    <span class="text-[10px] font-bold uppercase text-gray-400">Invoice Number</span>
                    <p class="text-lg font-bold text-brand-500">{{ $billing->invoice_number }}</p>
                </div>
            </div>
            ...
            <div class="p-7 space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    <div>
                        ...
                        <select name="client_id" required class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition dark:border-gray-700 dark:text-white/90">
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" {{ $billing->client_id == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="status" required class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition dark:border-gray-700 dark:text-white/90">
                            @foreach(['Pending', 'Sent', 'Paid', 'Overdue', 'Cancelled'] as $st)
                                <option value="{{ $st }}" {{ $billing->status == $st ? 'selected' : '' }}>{{ $st }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div x-data="{ 
            items: {{ $billing->items->count() > 0 ? $billing->items->map(fn($item) => ['description' => $item->description, 'quantity' => $item->quantity, 'unit_price' => $item->unit_price])->toJson() : json_encode([['description' => '', 'quantity' => 1, 'unit_price' => 0]]) }},
