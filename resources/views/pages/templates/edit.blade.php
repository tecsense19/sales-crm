            ...
            <div class="p-7 space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    <div class="sm:col-span-1">
                        ...
                        <input type="text" name="name" placeholder="e.g. New lead introduction" value="{{ old('name', $template->name) }}" required
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <select name="type" class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition dark:border-gray-700 dark:text-white/90">
                            <option value="email" {{ old('type', $template->type) == 'email' ? 'selected' : '' }}>Email Template</option>
                            <option value="whatsapp" {{ old('type', $template->type) == 'whatsapp' ? 'selected' : '' }}>WhatsApp Template</option>
                            <option value="general" {{ old('type', $template->type) == 'general' ? 'selected' : '' }}>General Message Template</option>
                        </select>
                    </div>
                    ...
                    <div class="sm:col-span-1">
                        ...
                        <input type="text" name="subject" placeholder="Default email subject (optional)" value="{{ old('subject', $template->subject) }}"
                            class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition dark:border-gray-700 dark:text-white/90" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" rows="12" placeholder="Hi {client_name}, ..." required
                        class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white/90 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition font-mono">{{ old('content', $template->content) }}</textarea>
