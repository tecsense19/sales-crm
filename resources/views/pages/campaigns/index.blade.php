    ...
    class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="border-b border-gray-200 px-5 py-4 dark:border-gray-800">
            <div class="flex items-center justify-between gap-3">
                <form action="{{ route('campaigns.index') }}" method="GET" class="flex flex-wrap items-center gap-2">
                    @if(request('status'))
                    ...
                    <div class="relative w-[260px]">
                        ...
                        @if(request('search'))
                            <a href="{{ route('campaigns.index', request()->except(['search', 'page'])) }}" class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
                    ...
                    @if(request('search'))
                        <a href="{{ route('campaigns.index', request()->except(['search', 'page'])) }}" class="shadow-theme-xs inline-flex h-10 items-center justify-center gap-1.5 rounded-lg border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="overflow-x-auto">
            <table class="w-full table-auto">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <td class="px-5 py-4 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @if($campaign->status !== 'Processing')
                                    @if($campaign->status !== 'Sent' || $campaign->pending_count > 0)
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-5 py-20 text-center text-gray-400 text-sm italic">No campaigns found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
