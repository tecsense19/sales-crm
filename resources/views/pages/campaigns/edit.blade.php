...
@section('content')
    ...
    <form action="{{ route('campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data" 
        x-data="{ 
            ...
            selectedClients: {{ json_encode($campaign->selected_clients ?? []) }},
            fileName: ''
        }" 
            ...
            <div class="p-7 space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="subject" value="{{ $campaign->subject }}" placeholder="What your clients will see in their inbox" required
                            class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition dark:border-gray-700 dark:text-white/90" />
                    </div>
                </div>
            </div>
            ...
            <div class="p-7 space-y-4">
                <div>
                    ...
                    <textarea name="content" rows="12" placeholder="Hello @{{name}}, we have a special offer for you..." required
                        class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:text-white/90 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 transition">{{ $campaign->body }}</textarea>
