        ...
        <div class="max-w-full overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-white/[0.05]">
                    <tr>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-start">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider">Deal ID</span>
                            </div>
                        </th>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-start">Customer</th>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-start">Product/Service</th>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-start">Deal Value</th>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-start">Close Date</th>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-start text-center">Status</th>
                        <th class="px-4 py-3 font-bold text-gray-800 text-[11px] dark:text-gray-200 uppercase tracking-wider text-end">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <template x-for="row in tableRowData" :key="row.id">
                        <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50/50 transition-colors">
                            <td class="px-4 py-2.5">
                                <div class="flex items-center gap-3">
                                    <div @click="handleRowSelect(row.id)"
                                        class="flex h-4 w-4 cursor-pointer items-center justify-center rounded border-[1.25px]"
                                        :class="selectedRows.includes(row.id) ? 'border-blue-500 bg-blue-500' : 'bg-white dark:bg-white/0 border-gray-300 dark:border-gray-700'">
                                        <svg :class="selectedRows.includes(row.id) ? 'block' : 'hidden'" width="10" height="10" viewBox="0 0 14 14" fill="none">
                                            <path d="M11.6668 3.5L5.25016 9.91667L2.3335 7" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    </div>
                                    <div>
                                        <span class="block font-bold text-brand-500 text-xs" x-text="row.id"></span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-2.5">
                                <div class="flex items-center gap-3">
                                    <div class="flex items-center justify-center w-8 h-8 rounded-full font-bold text-xs"
                                        :class="[row.avatarBg, row.avatarColor]">
                                        <span x-text="row.initials"></span>
                                    </div>
                                    <div>
                                        <span class="block text-sm font-bold text-gray-800 dark:text-white/90 leading-tight" x-text="row.customerName"></span>
                                        <span class="text-[11px] text-gray-400" x-text="row.customerEmail"></span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-2.5">
                                <p class="text-gray-600 text-xs dark:text-gray-400" x-text="row.product"></p>
                            </td>
                            <td class="px-4 py-2.5">
                                <p class="text-gray-800 font-bold text-sm dark:text-white/90" x-text="row.value"></p>
                            </td>
                            <td class="px-4 py-2.5">
                                <p class="text-gray-500 text-xs dark:text-gray-400" x-text="row.closeDate"></p>
                            </td>
                            <td class="px-4 py-2.5 text-center">
                                <span class="text-[10px] inline-block rounded-full px-2 py-0.5 font-bold uppercase" 
                                    :class="getStatusClass(row.status)" 
                                    x-text="row.status"></span>
                            </td>
                            <td class="px-4 py-2.5 text-end">
                                <button @click="deleteRow(row.id)" class="text-gray-400 hover:text-red-500 transition-colors">
